...
use super::{Batch, Error, FaultKind, DumboBuilder, Message, Result};
use crate::{ConsensusProtocol, Contribution, Fault, NetworkInfo, NodeIdT};

use super::Params;

/// An instance of the Dumbo Byzantine fault tolerant consensus algorithm.
#[derive(Derivative)]
#[derivative(Debug)]
pub struct Dumbo<C, N> {
    /// Shared network data.
...
/// A `Dumbo` step, possibly containing multiple outputs.
pub type Step<C, N> = crate::CpStep<Dumbo<C, N>>;

impl<C, N> ConsensusProtocol for Dumbo<C, N>
where
...
where
    ...
    N: NodeIdT,
{
    /// Returns a new `DumboBuilder` configured to use the node IDs and cryptographic keys
    /// specified by `netinfo`.
    pub fn builder(netinfo: Arc<NetworkInfo<N>>) -> DumboBuilder<C, N> {
        DumboBuilder::new(netinfo)
    }
    ...
    /// Returns the parameters controlling Dumbo's behavior and performance.
