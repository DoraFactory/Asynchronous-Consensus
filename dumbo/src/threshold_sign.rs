...
use hbbft::fault_log::{Fault, FaultLog};
...
impl<N: NodeIdT> ConsensusProtocol for ThresholdSign<N> {
    ...
    fn handle_input<R: Rng>(&mut self, sender_id: &N, _rng: &mut R) -> Result<Step<N>> {
        self.sign(sender_id)
    }
...
impl<N: NodeIdT> ThresholdSign<N> {
    ...
    pub fn sign(&mut self, sender_id: &N) -> Result<Step<N>> {
        if self.had_input {
        ...
        // 将消息签名发送给sender
        step.messages.push(Target::Node(sender_id).message(msg.clone()));
