...
use super::{
    Batch, Change, ChangeState, DynamicDumboBuilder, EncryptionSchedule, Error, FaultKind,
...
use crate::dumbo::{self, Dumbo, Message as HbMessage};
...
/// A Dumbo instance that can handle adding and removing nodes.
#[derive(Derivative)]
#[derivative(Debug)]
pub struct DynamicDumbo<C, N: Ord> {
    /// Shared network data.
    ...
    /// The `Dumbo` instance with the current set of nodes.
    pub(super) dumbo: Dumbo<InternalContrib<C, N>, N>,
    /// The current key generation process, and the change it applies to.
    pub(super) key_gen_state: Option<KeyGenState<N>>,
}

impl<C, N> ConsensusProtocol for DynamicDumbo<C, N>
where
    ...
    N: NodeIdT + Serialize + DeserializeOwned,
{
    ...
    fn handle_input<R: Rng>(&mut self, input: Self::Input, rng: &mut R) -> Result<Step<C, N>> {
        // User contributions are forwarded to `Dumbo` right away. Votes are signed and
        // broadcast.
        match input {
...
impl<C, N> DynamicDumbo<C, N>
where
    C: Contribution + Serialize + DeserializeOwned,
    N: NodeIdT + Serialize + DeserializeOwned,
{
    /// Returns a new `DynamicDumboBuilder`.
    pub fn builder() -> DynamicDumboBuilder<C, N> {
        DynamicDumboBuilder::new()
    }

    /// Creates a new `DynamicDumbo` ready to join the network specified in the `JoinPlan`.
    pub fn new_joining<R: Rng>(
        ...
        let dumbo = Dumbo::builder(arc_netinfo.clone())
            .session_id(join_plan.era)
            .params(join_plan.params)
            .build();
        let mut dhb = DynamicDumbo {
            netinfo,
            max_future_epochs,
            era: join_plan.era,
    ...
    pub fn handle_message<R: Rng>(
        ...
        if message.era() == self.era {
            match message {
                Message::Dumbo(_, hb_msg) => {
                    self.handle_dumbo_message(sender_id, hb_msg, rng)
                }
    ...
    pub fn should_propose(&self) -> bool {
        ...
        if self.dumbo.received_proposals() > self.netinfo.num_faulty() {
    ...
    pub fn next_epoch(&self) -> u64 {
        self.era + self.dumbo.next_epoch()
    }

    /// Handles a message for the `Dumbo` instance.
    fn handle_dumbo_message<R: Rng>(
        &mut self,
        sender_id: &N,
        message: HbMessage<N>,
        rng: &mut R,
        ...
        let step = self
            ...
            .map_err(Error::HandleDumboMessage)?;
        self.process_output(step, rng)
    }

    /// Handles a vote or key generation message and tries to commit it as a transaction. These
    /// messages are only handled once they appear in a batch output from Dumbo.
    ...
    fn process_output<R: Rng>(
        ...
        for hb_batch in output {
            ...
            let change = if let Some(kgs) = self.take_ready_key_gen() {
                // If DKG completed, apply the change, restart Dumbo, and inform the user.
                debug!("{}: DKG for complete for: {:?}", self, kgs.public_keys());
                self.netinfo = kgs.key_gen.into_network_info().map_err(Error::SyncKeyGen)?;
                let params = self.dumbo.params().clone();
                self.restart_dumbo(batch_epoch + 1, params);
            ...
            step.output.push(Batch {
                ...
                params: self.dumbo.params().clone(),
            });
        }
        Ok(step)
    }

    /// Restarts Dumbo with the new encryption schedule.
    pub(super) fn update_encryption_schedule(&mut self, era: u64, schedule: EncryptionSchedule) {
        let mut params = self.dumbo.params().clone();
        params.encryption_schedule = schedule;
        self.restart_dumbo(era, params);
    }
    ...
    /// Starts a new `Dumbo` instance and resets the vote counter.
    fn restart_dumbo(&mut self, era: u64, params: Params) {
        self.era = era;
        self.key_gen_msg_buffer.retain(|kg_msg| kg_msg.0 >= era);
        let netinfo = Arc::new(self.netinfo.clone());
        self.vote_counter = VoteCounter::new(netinfo.clone(), era);
        self.dumbo = Dumbo::builder(netinfo)
            .session_id(era)
            .params(params)
            .build();
    }

    /// Handles a `Part` message that was output by Dumbo.
    ...
    /// Handles an `Ack` message that was output by Dumbo.
    ...
    /// Returns the maximum future epochs of the Dumbo algorithm instance.
    pub fn max_future_epochs(&self) -> u64 {
        self.max_future_epochs
    }
}

impl<C, N> fmt::Display for DynamicDumbo<C, N>
where
    C: Contribution + Serialize + DeserializeOwned,
    N: NodeIdT + Serialize + DeserializeOwned,
{
...
impl<C, N> Epoched for DynamicDumbo<C, N>
where
    C: Contribution + Serialize + DeserializeOwned,
    N: NodeIdT + Serialize + DeserializeOwned,
{
