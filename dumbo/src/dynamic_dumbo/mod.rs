//! # Dynamic Dumbo
//!
//! Like Dumbo, this protocol allows a network of _N_ nodes with at most _f_ faulty ones,
...
//! exactly one instance of `Dumbo`. Each following era begins immediately after a batch that
//!
...
//!
//! Unlike Dumbo, this algorithm allows dynamically adding and removing validators.
...
//! `DynamicDumbo` instance created from a `JoinPlan` will start as an observer in the
...
//! ## How it works
//!
//! Dynamic Dumbo runs a regular Dumbo instance internally, which in addition to the
...
//!
//! Every time Dumbo outputs a new batch, Dynamic Dumbo outputs the user
...
//! change begins. If key generation completes successfully, the Dumbo instance is dropped,
...
use crate::dumbo::{EncryptionSchedule, Message as HbMessage, Params};
use crate::sync_key_gen::{Ack, Part, SyncKeyGen};
use crate::NodeIdT;

pub use self::batch::Batch;
pub use self::builder::DynamicDumboBuilder;
pub use self::change::{Change, ChangeState};
pub use self::dynamic_dumbo::DynamicDumbo;
pub use self::error::{Error, FaultKind, Result};

/// A `DynamicDumbo` step, possibly containing multiple outputs.
pub type Step<C, N> = crate::CpStep<DynamicDumbo<C, N>>;

/// The user input for `DynamicDumbo`.
#[derive(Clone, Debug)]
...
/// generation. It gets committed via Dumbo and is only handled after it has been output in
...
/// A message sent to or received from another node's Dumbo instance.
#[derive(Serialize, Deserialize, Debug, Clone)]
pub enum Message<N: Ord> {
    /// A message belonging to the `Dumbo` algorithm started in the given epoch.
    Dumbo(u64, HbMessage<N>),
...
/// The contribution for the internal `Dumbo` instance: this includes a user-defined
...
struct InternalContrib<C, N: Ord> {
    ...
    contrib: C,
    /// Key generation messages that get committed via Dumbo to communicate synchronously.
